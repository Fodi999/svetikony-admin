          ...
          <div className="space-y-3">
            {slot.imageUrl ? (
              <div className="space-y-1">
                ...
                {preview.kind === "photo_then_text" && preview.photoCaption ? <MessageBubble>{preview.photoCaption}</MessageBubble> : null}
              </div>
            ) : null}

            {preview.kind === "text_only" || preview.kind === "photo_then_text" ? <MessageBubble>{slot.fullText}</MessageBubble> : null}

            <p className="text-xs text-muted-foreground">
              {preview.kind === "text_only" && "Одне текстове повідомлення (без фото)."}
