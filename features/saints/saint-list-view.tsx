...
export function SaintListView() {
  ...
  const [pendingDelete, setPendingDelete] = useState<Saint | null>(null);

  const query = useQuery({
    queryKey: ["saints", { search, language, status }],
    queryFn: () =>
      apiClient.saints.list({
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.saints.remove(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">{messages.nav.saints}</h1>
          <p className="text-sm text-muted-foreground">Житія святих.</p>
        </div>
        {editable ? (
          <GuardedLink href="/saints/new" className={cn(buttonVariants(), "hidden md:inline-flex")}>
      ...
      {query.isLoading ? (
        ...
        <>
          <div className="space-y-3 md:hidden">
            {items.map((saint) => (
              <GuardedLink key={saint.id} href={`/saints/${saint.id}`} className="block">
                <Card className="transition-colors hover:bg-accent/50">
                  <CardContent className="space-y-2 p-4">
                    ...
                    <Badge variant="outline">{LANGUAGE_LABELS[saint.language]}</Badge>
                  </CardContent>
                </Card>
              </GuardedLink>
            ))}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>День памʼяті</TableHead>
              ...
              <TableBody>
                {items.map((saint) => (
                  <TableRow key={saint.id}>
                    <TableCell>
                      <GuardedLink href={`/saints/${saint.id}`} className="block font-medium">
                        {saint.name}
                      </GuardedLink>
                    </TableCell>
