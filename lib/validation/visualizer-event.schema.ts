...
export const visualizerEventTypeSchema = z.enum([
  "biblical",
  "church_history",
  "historical",
  "saint",
...
export const visualizerChronologyTypeSchema = z.enum([
  "exact",
  "approximate",
  "traditional",
  "period",
...
export const visualizerEventSchema = z
  .object({
    slug: slugSchema,
    ...
    yearStart: z.number().int().positive().optional(),
    yearEnd: z.number().int().positive().optional(),
  ...
  .superRefine((event, context) => {
    if (event.yearStart != null && event.yearEnd != null) {
      const sign = event.calendarEra === "BC" ? -1 : 1;
      if (sign * event.yearEnd < sign * event.yearStart)
        context.addIssue({
          code: "custom",
          path: ["yearEnd"],
          message: "Кінець періоду має бути після початку",
        });
    }
  });
