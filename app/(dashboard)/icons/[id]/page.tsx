...
export default function EditIconPage() {
  ...
  const query = useQuery({
    queryKey: ["icons", params.id],
    queryFn: () => apiClient.icons.get(params.id),
  });

  const updateMutation = useMutation({
    mutationFn: (values: IconFormValues) => apiClient.icons.update(params.id, values),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => apiClient.icons.remove(params.id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["icons"] });
