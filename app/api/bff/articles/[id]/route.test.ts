...
describe("GET/PUT/DELETE /api/bff/articles/:id", () => {
  ...
  describe("GET", () => {
    it("never returns internal Worker fields, and does forward the real iconId", async () => {
      vi.stubGlobal(
        "fetch",
      ...
      const response = await GET(new NextRequest("http://localhost/api/bff/articles/article-1", withSessionCookie()), {
      ...
      expect(body.iconId).toBe("icon-1");
    });
    ...
    it("URL-encodes the id in the upstream path", async () => {
      ...
      await GET(new NextRequest("http://localhost/api/bff/articles/id with space", withSessionCookie()), {
    ...
    it("passes through a 404 unchanged", async () => {
      vi.stubGlobal(
        "fetch",
        mockAuthenticatedFetch("viewer", () =>
          new Response(JSON.stringify({ code: "NOT_FOUND", message: "article not found" }), { status: 404, headers: { "content-type": "application/json" } }),
        ),
      );
      const response = await GET(new NextRequest("http://localhost/api/bff/articles/missing", withSessionCookie()), {
  ...
  describe("PUT", () => {
    it("an editor can update an article", async () => {
      const fetchMock = mockAuthenticatedFetch("editor", () =>
        new Response(JSON.stringify({ ...workerArticle, title: "Оновлено" }), { status: 200, headers: { "content-type": "application/json" } }),
      );
      vi.stubGlobal("fetch", fetchMock);
      const response = await PUT(putRequest({ title: "Оновлено" }), { params: Promise.resolve({ id: "article-1" }) });
      expect(response.status).toBe(200);
      const body = (await response.json()) as { title: string };
      expect(body.title).toBe("Оновлено");
    });
    ...
    it("a viewer is rejected with 403, and the real article upstream is never called", async () => {
      ...
      vi.stubGlobal("fetch", fetchMock);
      const response = await PUT(putRequest({ title: "x" }), { params: Promise.resolve({ id: "article-1" }) });
  ...
  describe("DELETE", () => {
    it("an editor can delete an article (real hard delete, not archive)", async () => {
      vi.stubGlobal("fetch", mockAuthenticatedFetch("editor", () => new Response(null, { status: 204 })));
      const response = await DELETE(
        new NextRequest("http://localhost/api/bff/articles/article-1", withSessionCookie({ method: "DELETE" })),
        { params: Promise.resolve({ id: "article-1" }) },
      );
    ...
    it("a super_admin can delete an article", async () => {
