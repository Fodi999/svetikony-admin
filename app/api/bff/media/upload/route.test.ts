...
import { isAuditCall, mockAuthenticatedFetch, withSessionCookie } from "../../_lib/test-support";
...
function uploadRequest(fields: Record<string, string | Blob>) {
  ...
  return new NextRequest("http://localhost/api/bff/media/upload", withSessionCookie({ method: "POST", body: form }));
}
...
describe("POST /api/bff/media/upload", () => {
  ...
  it("forwards the multipart body and attaches the server-side token, without exposing it back", async () => {
    ...
    // fetchMock is called twice: once for withAuth's own session check,
    // once for the resource's own upstream upload -- assert on the real
    // (non-session) call specifically rather than call count.
    const uploadCall = fetchMock.mock.calls.find(([url]) => !String(url).includes("/api/admin/auth/session") && !isAuditCall(url))!;
  ...
  it("returns a 401 with no-store when the service token is missing, without leaking the missing value", async () => {
    // withAuth's own session resolution needs SVET_IKONY_ADMIN_TOKEN too
    // (see app/api/bff/_lib/auth.ts's resolveSession) -- deleting it means
    // the request never gets past the auth check at all, which is the
    // correct/expected 401 here (not a claim about the resource's own
    // upstream call specifically).
  ...
  it("returns a 502 on network failure without a stack trace", async () => {
    vi.stubGlobal("fetch", mockAuthenticatedFetch("editor", () => Promise.reject(new TypeError("fetch failed"))));
    const file = new File([new Uint8Array([1])], "a.jpg", { type: "image/jpeg" });
    const response = await POST(uploadRequest({ file, module: "alphabet", entityId: "x", purpose: "card" }));
  ...
  it("returns a 502 on upstream timeout (abort)", async () => {
    vi.stubGlobal(
      "fetch",
      mockAuthenticatedFetch("editor", () => Promise.reject(Object.assign(new DOMException("aborted", "AbortError")))),
    );
    const file = new File([new Uint8Array([1])], "a.jpg", { type: "image/jpeg" });
    const response = await POST(uploadRequest({ file, module: "alphabet", entityId: "x", purpose: "card" }));
    expect(response.status).toBe(502);
  ...
  it("returns 400 when the body is not valid multipart form data", async () => {
    vi.stubGlobal("fetch", mockAuthenticatedFetch("editor", () => new Response("[]", { status: 200 })));
    const response = await POST(
      new NextRequest("http://localhost/api/bff/media/upload", withSessionCookie({ method: "POST", body: "not-multipart" })),
    );
