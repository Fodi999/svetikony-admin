...
/** Proxies the Content Plan "image" slot action -- see
 * svet-ikony's app/api/admin/telegram/content-plan/[date]/[type]/image/route.ts. */
export async function PUT(request: Request, { params }: { params: Promise<{ date: string; type: string }> }) {
  ...
  return proxyJsonWrite(
    `${UPSTREAM_ENDPOINTS.telegram.contentPlan}/${encodeURIComponent(date)}/${encodeURIComponent(type)}/image`,
    "PUT",
    payload,
