...
/** Proxies the Content Plan "regenerate-image" slot action -- see
 * svet-ikony's app/api/admin/telegram/content-plan/[date]/[type]/regenerate-image/route.ts.
...
export async function POST(_request: Request, { params }: { params: Promise<{ date: string; type: string }> }) {
  ...
  return proxyJsonWrite(
    `${UPSTREAM_ENDPOINTS.telegram.contentPlan}/${encodeURIComponent(date)}/${encodeURIComponent(type)}/regenerate-image`,
    "POST",
    undefined,
