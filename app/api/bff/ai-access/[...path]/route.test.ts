...
vi.mock("../../_lib/auth", () => ({
  withAuth: (_policy: unknown, handler: (...args: unknown[]) => unknown) =>
    (request: unknown, context: unknown) => handler(request, {}, context),
}));

const context = { params: Promise.resolve({ path: ["grants"] }) };
function request() {
  return new NextRequest("https://admin.example/api/bff/ai-access/grants", {
    headers: { cookie: "admin_session=fixture-session" },
  });
}

describe("AI access BFF Workers transport", () => {
  beforeEach(() => {
  ...
  it("loads grants using a Workers-compatible redirect policy", async () => {
    const fetchMock = vi.fn(async (_url: unknown, init: RequestInit) => {
      ...
      expect(init.headers).toMatchObject({
        Authorization: "Bearer fixture-service-token", "X-Admin-Session": "fixture-session",
      });
  ...
  it.each([301, 302, 307, 308])("rejects upstream %s without following or exposing Location", async (status) => {
